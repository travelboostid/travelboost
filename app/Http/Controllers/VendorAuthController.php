...
class VendorAuthController extends Controller
{
  ...
  public function login(Request $request)
  {
    ...
    $vendorTeam = CompanyTeam::where('user_id', $user->id)
      ->whereHas('company', function ($query) {
        $query->where('type', CompanyType::VENDOR);
      })
      ->with('company')
      ->first();
    ...
    if (!$vendorTeam) {
      return back()->withErrors([
        'access_denied' => 'Your account is not registered as a Vendor in Travelboost. Please make sure you are logging into the correct portal or register as a Vendor to continue.',
      ]);
    }
    ...
    return redirect()->route('company.index', ['company' => $vendorTeam->company->username]);
  }
