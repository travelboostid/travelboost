...
class ChatbotAgent implements Agent, Conversational
{
    ...
    private function retrieveGeneralContext(array $args)
    {
        ...
        $generalKnowledgeBaseDocumentIds = Media::query()
        ...
        $documents = KnowledgeBase::query()
            ...
            ->whereIn('owner_id', $generalKnowledgeBaseDocumentIds)
            ->limit(3)
            ->pluck('content');
        ...
        $relevantKnowledges = $documents->map(function ($doc) {
            return "- {$doc}";
        })->implode("\n");
    ...
    private function retrieveTourDetailContext(array $args)
    {
        ...
        $targetKnowledgeBaseDocumentIds = collect([$tour->document_id])->merge($generalKnowledgeBaseDocumentIds)->filter()->values();
        $documents = KnowledgeBase::query()
            ->whereVectorSimilarTo('embedding', $embedded->embeddings[0] ?? null, minSimilarity: 0.1)
            ->where('owner_type', Media::class)
            ->whereIn('owner_id', $targetKnowledgeBaseDocumentIds)
            ->limit(3)
            ->pluck('content');
    ...
    private function retrieveMyBookingsContext(array $args)
    {
        $bookings = Booking::query()
            ...
            ->when(($args['tour_id'] ?? 0) > 0, fn ($q) => $q->where('tour_id', $args['tour_id']))
