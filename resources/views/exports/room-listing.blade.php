...
<body style="{{ $isExcel ? 'font-family: Arial, sans-serif; font-size: 10px; color: #0f172a;' : '' }}">
  @if($isExcel)
  <table style="width: 100%;">
    <tr>
      ...
      <td colspan="5" style="font-size: 10pt; font-weight: 700; color: #0f172a; vertical-align: middle;">{{ $tour ? ($tour->code . ' - ' . $tour->name) : 'Not selected' }}</td>
    </tr>
    <tr>
      ...
      <td colspan="5" style="font-size: 10pt; font-weight: 700; color: #0f172a; vertical-align: middle;">{{ $departure_date ? \Carbon\Carbon::parse($departure_date)->format('d F Y') : 'Not selected' }}</td>
    </tr>
    <tr>
  ...
  <table cellpadding="0" cellspacing="0" class="page-header">
    <tr>
        ...
        <table cellpadding="0" cellspacing="0" style="width: 100%;">
          <tr>
            ...
            <td style="width: 68px; vertical-align: middle;">
              <img src="{{ public_path($company->photo_url) }}" class="logo" alt="Company logo">
            </td>
            @endif
      ...
      <td style="width: 35%; text-align: right;">
        ...
        <div class="meta-value">{{ $tour ? ($tour->code . ' - ' . $tour->name) : 'Not selected' }}</div>
        <div class="meta-label" style="margin-top: 8px;">Departure Date</div>
        <div class="meta-value">{{ $departure_date ? \Carbon\Carbon::parse($departure_date)->format('d F Y') : 'Not selected' }}</div>
      </td>
    </tr>
  </table>
  @endif
  ...
  <table style="border-collapse: collapse; width: 100%; table-layout: fixed;">
    <thead>
      <tr>
        ...
        @endphp
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="3%"' : '' !!}>No</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="18%"' : '' !!}>Passenger Name</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="8%"' : '' !!}>Room Type</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Room No.</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Room</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Seat</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Visa</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="14%"' : '' !!}>Remarks</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="10%"' : '' !!}>Passport Number</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="8%"' : '' !!}>Issue Date</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="8%"' : '' !!}>Expiry Date</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="8%"' : '' !!}>Place of Birth</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="8%"' : '' !!}>Date of Birth</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="9%"' : '' !!}>Contact</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Age</th>
        <th style="{{ $headerStyle }}" {!! !$isExcel ? 'width="4%"' : '' !!}>Val</th>
      </tr>
    </thead>
    <tbody>
      @php
      ...
      @foreach($groupedData as $bookingNumber => $bookingData)
        ...
        @foreach($bookingData['rooms'] as $roomGroup)
          ...
          @foreach($passengers as $passenger)
            ...
            @endphp
            <tr>
              <td style="{{ $cellStyle }} text-align: center;">{{ $globalIndex++ }}</td>
              <td style="{{ $cellStyle }} font-weight: 700;">{{ $fullName }}</td>
              ...
              <td style="{{ $cellStyle }} text-transform: uppercase;">{{ $passenger->pob ?: '-' }}</td>
              <td style="{{ $cellStyle }} text-align: center;">{{ $passenger->dob ? \Carbon\Carbon::parse($passenger->dob)->format('d/m/Y') : '-' }}</td>

              @if($isFirstInBooking)
                <td rowspan="{{ $bookingData['total_pax'] }}" style="{{ $cellStyle }} text-align: center; font-size: 8pt; mso-number-format: '\@';">{{ $bookingData['contact_phone'] ?: '-' }}</td>
              @endif

              <td style="{{ $cellStyle }} text-align: center;">{{ $age }}</td>
              <td style="{{ $cellStyle }} text-align: center; font-weight: 700; {{ $isWarning ? 'background-color: #fee2e2; color: #dc2626;' : '' }}">{{ $validityMonths }}</td>
            </tr>
            @php
