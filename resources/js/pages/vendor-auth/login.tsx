...
import { login as loginAsAgent } from '@/routes/agent';
import {
  login as loginAsVendor,
  register as registerAsVendor,
} from '@/routes/vendor';
...
export default function VendorLogin({ status }: Props) {
      ...
      >
        ...
        <form onSubmit={submit} className="flex flex-col gap-6">
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            <TextLink href={registerAsVendor()} tabIndex={5}>
              Sign up
            </TextLink>
          </div>
        </form>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-background p-6 rounded-lg shadow-xl max-w-sm w-full text-center border border-border">
            ...
            <p className="text-sm text-muted-foreground mb-2">{modalMessage}</p>
            <p className="text-sm text-muted-foreground mb-6">
              Your account has been deactivated, contact the Travelboost admin
              at care@travelboost.co.id for further information.
            </p>
