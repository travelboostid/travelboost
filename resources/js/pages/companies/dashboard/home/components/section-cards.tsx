...
function CurrencyAmount({ value }: { value: string }) {
  ...
  return (
    <div className="flex min-w-0 items-baseline gap-1.5">
      ...
      <span
        className={`min-w-0 whitespace-nowrap font-bold leading-tight text-slate-900 tabular-nums dark:text-slate-100 ${getCurrencyTextClass(value)}`}
      >
        {normalizedValue}
      </span>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      {metrics.map((m, i) => (
        <div
          key={i}
          className="flex min-w-0 items-center gap-5 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800"
        >
          <div className={`shrink-0 rounded-2xl p-4 ${m.color}`}>{m.icon}</div>
          <div className="min-w-0">
            ...
            {m.isCurrency ? (
              ...
              <h3 className="whitespace-nowrap text-xl font-bold leading-tight text-slate-900 tabular-nums dark:text-slate-100">
                {m.value}
              </h3>
            )}
