...
type PendingTopupConfirmDialogProps = {
    ...
    pendingTopup: NonNullable<ChatbotPageProps['pendingTopup']>;
    onStartNew: () => void;
};

function openAiCreditTopupPayment(
    payment: NonNullable<ChatbotPageProps['pendingTopup']>,
): void {
    openOnlinePayment(
        {
...
export default function PendingTopupConfirmDialog({
    ...
    const handleContinue = () => {
        ...
        window.setTimeout(() => {
            openAiCreditTopupPayment(pendingTopup);
        }, 100);
    };
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-2">
                            ...
                            <AlertDialogDescription className="text-sm leading-relaxed">
                                {amount !== null ? (
                                    <FormattedMessage
                                        defaultMessage="You already have a pending AI top-up of {amount}. Continue that payment or cancel it to start a new one."
                                        values={{ amount: formatIDR(amount) }}
                                    />
                                ) : (
                                    <FormattedMessage defaultMessage="You already have a pending AI top-up. Continue that payment or cancel it to start a new one." />
                                )}
                            </AlertDialogDescription>
                        </div>
                    </div>
