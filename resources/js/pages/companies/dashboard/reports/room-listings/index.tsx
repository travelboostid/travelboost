...
const TourAutocomplete = ({
  ...
  const selectedTourLabel = useMemo(() => {
    ...
    const tour = tours.find((item) => item.id.toString() === value.toString());
      ...
      {open && filteredTours.length > 0 && (
        <ul className="absolute z-50 mt-2 max-h-60 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-900">
          {filteredTours.map((tour) => (
            ...
            >
              <span className="font-semibold">{tour.code}</span> - {tour.name}
            </li>
          ))}
        </ul>
      )}
                  ...
                  {Object.entries(groupedData).map(
                    ([bookingNumber, roomGroups]) => {
                      ...
                      return Object.entries(roomGroups).map(
                        ([roomType, passengerList], roomIndex) => {
                          return passengerList.map(
                            (row: any, passengerIndex: number) => {
                              const isFirstInBooking =
                                roomIndex === 0 && passengerIndex === 0;
                              const isFirstInRoom = passengerIndex === 0;
                              const totalPaxInRoom = passengerList.length;
                              const fullName =
                                `${row.title ? row.title + '. ' : ''}${row.first_name} ${row.last_name || ''}`.trim();
                              const validityMonths = calculateValidity(
