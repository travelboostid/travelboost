...
test('payment methods can be filtered by platform usage scope', function () {
    $user = User::factory()->create();
    ...
    $platformMethod = PaymentMethod::query()->create([
        'provider' => 'midtrans',
        'method' => 'bca_va',
        'name' => 'Midtrans BCA',
        'description' => 'Platform VA',
    ...
    $response = $this->actingAs($user)
        ->getJson('/webapi/payment-methods?usage_scope=platform');

    $response->assertOk();

    $ids = collect($response->json('data'))->pluck('id')->all();

    expect($ids)->toBe([$platformMethod->id]);
});
